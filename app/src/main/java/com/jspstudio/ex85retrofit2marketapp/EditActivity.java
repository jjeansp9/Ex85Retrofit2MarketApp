...
import com.jspstudio.ex85retrofit2marketapp.databinding.ActivityEditBinding;
...
public class EditActivity extends AppCompatActivity {
    ...
    ActivityResultLauncher<Intent> resultLauncher= registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), new ActivityResultCallback<ActivityResult>() {
        ...
        public void onActivityResult(ActivityResult result) {
            if(result.getResultCode()!=RESULT_CANCELED){
                ...
                // uri (콘텐츠 DB주소) 확인해보기
                //new AlertDialog.Builder(EditActivity.this).setMessage(uri.toString()).show(); // create() 생략해도 show()만 하면 create()도 알아서 해줌

                // 레트로핏을 통해 서버에 사진파일을 보내려면 파일의 콘텐츠경로인 uri가 아니라 실제 경로인 절대주소가 필요
                // uri --> 절대주소로 변환하기기 [ 단, 외부저장소에 대한 퍼미션 필요 ]
    ...
    void clickComplete(){
        ...
        // 서버에 보낼 데이터들을 택배상자에 포장 [ 파일박스, 데이터박스 ]
        // 1. 파일 박스
        ...
        call.enqueue(new Callback<String>() {
            ...
            public void onResponse(Call<String> call, Response<String> response) {
                ...
                // 업로드가 완료되었으므로 액티비티를 종료하기(글 작성화면이 없어짐)
                finish();
            }
